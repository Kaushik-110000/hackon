...
interface Manufacturing {
  ...
  energyType?: keyof CarbonFootprintCalculator['emissionFactors']['energy'];
}
...
class CarbonFootprintCalculator {
  ...
  calculateRawMaterialsFootprint(materials: Material[]): number {
  ...
  calculateManufacturingFootprint(energyConsumption: number, energyType: string = 'grid'): number {
    const factor = this.emissionFactors.energy[energyType] || this.emissionFactors.energy.grid;
    return energyConsumption * factor;
  }

  calculateTransportationFootprint(routes: Route[]): number {
    let totalFootprint = 0;
  ...
  calculatePackagingFootprint(packagingType: string, weight: number): number {
    const factor = this.emissionFactors.packaging[packagingType] || 1.0;
    return weight * factor;
  }

  calculateDisposalFootprint(weight: number, isRecyclable: boolean, isBiodegradable: boolean): number {
    if (isRecyclable) return weight * 0.1;
    if (isBiodegradable) return weight * 0.05;
    return weight * 0.8;
  }

  calculateProductFootprint(productData: ProductData): CarbonFootprint {
    const {
      materials = [],
      manufacturing = {},
      transportation = [],
  ...
  calculategreenScore(carbonFootprint: CarbonFootprint, sustainabilityFeatures: SustainabilityFeatures = {}): number {
  ...
  calculateGreenCoinsReward(greenScore: number, purchaseAmount: number): number {
